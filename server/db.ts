...
export async function getDb() {
  ...
  try {
    ...
    _connectionAttempts++
    console.error(`[Database] Failed to connect (attempt ${_connectionAttempts}/${MAX_CONNECTION_ATTEMPTS}):`, error)
    ...
    if (process.env.NODE_ENV === "production" && _connectionAttempts < MAX_CONNECTION_ATTEMPTS) {
      console.log(`[Database] Will retry connection in ${RETRY_DELAY_MS/1000} seconds...`)
      
      // Wait and try again
      await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS))
      return getDb() // Recursive call to retry
    } else if (_connectionAttempts >= MAX_CONNECTION_ATTEMPTS) {
      console.error(`[Database] Maximum connection attempts (${MAX_CONNECTION_ATTEMPTS}) reached. Giving up.`)
    }
