...
export async function upsertAdminCredential(email: string, password: string, name?: string): Promise<void> {
  const db = await getDb()
  if (!db) {
    console.warn("[Database] Cannot upsert admin credential: database not available")
    return
  }
...
export async function verifyAdminLogin(email: string, password: string): Promise<AdminCredential | null> {
  const db = await getDb()
  if (!db) {
  ...
  try {
    ...
    await db.update(adminCredentials).set({ lastLogin: new Date() }).where(eq(adminCredentials.id, admin.id))
...
export async function getAdminByEmail(email: string): Promise<AdminCredential | null> {
  const db = await getDb()
  if (!db) {
...
export async function updateAdminPassword(email: string, newPassword: string): Promise<boolean> {
  ...
  try {
    ...
    await db.update(adminCredentials).set({ passwordHash }).where(eq(adminCredentials.email, email))
    return true
  } catch (error) {
...
export async function updateAdminEmail(oldEmail: string, newEmail: string): Promise<boolean> {
  const db = await getDb()
  if (!db) {
  ...
  try {
    await db.update(adminCredentials).set({ email: newEmail }).where(eq(adminCredentials.email, oldEmail))
    return true
  } catch (error) {
