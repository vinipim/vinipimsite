...
export const reviewsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }: { input: any }) => {
      ...
      if (input.sortBy === "date") {
        query = query.orderBy(input.order === "desc" ? desc(reviews.createdAt) : reviews.createdAt) as any
      } else if (input.sortBy === "rating") {
        query = query.orderBy(input.order === "desc" ? desc(reviews.rating) : reviews.rating) as any
      } else if (input.sortBy === "title") {
        query = query.orderBy(input.order === "desc" ? desc(reviews.title) : reviews.title) as any
      }
  ...
  getById: protectedProcedure.input(z.object({ id: z.string() })).query(async ({ input }: { input: any }) => {
    const db = await getDb()
    if (!db) return null
  ...
  delete: protectedProcedure.input(z.object({ id: z.string() })).mutation(async ({ input }: { input: any }) => {
    const db = await getDb()
    if (!db) throw new Error("Database not available")
